...
App.Pages.Installation = function () {
  ...
  $install.on('click', function () {
    ...
    var url = App.Utils.Url.siteUrl('installation/perform');
    var data = {
    ...
    $.ajax({
      ...
      $alert.text('Easy!Appointments has been successfully installed!').addClass('alert-success').prop('hidden', false);
      setTimeout(function () {
  ...
  function validate() {
    try {
      ...
      $fields.removeClass('is-invalid'); // Check for empty fields.
      ...
      if ($password.val().length < MIN_PASSWORD_LENGTH) {
        $password.addClass('is-invalid');
        $retypePassword.addClass('is-invalid');
        throw new Error("The password must be at least ".concat(MIN_PASSWORD_LENGTH, " characters long."));
      } // Validate Email
  ...
  if (vars('base_url').slice(-1) === '/') {
    App.Utils.Message.show('Invalid Configuration Detected', 'Please remove any trailing slashes from your "BASE_URL" setting of the root "config.php" file and try again.');
