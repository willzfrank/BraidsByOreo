...
App.Pages.Customers = function () {
  ...
  var $filterCustomers = $('#filter-customers');
  var $id = $('#customer-id');
  ...
  function addEventListeners() {
    ...
    $customers.on('submit', '#filter-customers form', function (event) {
      ...
      var key = $filterCustomers.find('.key').val();
    ...
    $customers.on('click', '.customer-row', function (event) {
  ...
  function save(customer) {
    App.Http.Customers.save(customer).then(function (response) {
  ...
  function remove(id) {
    App.Http.Customers.destroy(id).then(function () {
  ...
  function display(customer) {
    ...
    customer.appointments.forEach(function (appointment) {
      if (vars('role_slug') === App.Layouts.Backend.DB_SLUG_PROVIDER && parseInt(appointment.id_users_provider) !== vars('user_id')) {
        return;
      }

      if (vars('role_slug') === App.Layouts.Backend.DB_SLUG_SECRETARY && vars('secretary_providers').indexOf(appointment.id_users_provider) === -1) {
        return;
      }

      var start = App.Utils.Date.format(moment(appointment.start_datetime).toDate(), vars('date_format'), vars('time_format'), true);
      var end = App.Utils.Date.format(moment(appointment.end_datetime).toDate(), vars('date_format'), vars('time_format'), true);
      $('<div/>', {
        'class': 'appointment-row',
        'data-id': appointment.id,
        'html': [// Service - Provider
        $('<a/>', {
          'href': App.Utils.Url.siteUrl("calendar/reschedule/".concat(appointment.hash)),
          'html': [$('<i/>', {
  ...
  function filter(keyword) {
    ...
    App.Http.Customers.search(keyword, filterLimit).then(function (response) {
      ...
      if (!response.length) {
        $filterCustomers.find('.results').append($('<em/>', {
          'text': lang('no_records_found')
        }));
