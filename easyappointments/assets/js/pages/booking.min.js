...
App.Pages.Booking = function () {
  ...
  var $bookAppointmentSubmit = $('#book-appointment-submit');
  var $deletePersonalInformation = $('#delete-personal-information');
  /**
  ...
  function initialize() {
    if (vars('display_cookie_notice')) {
      ...
      $cookieNoticeLink.replaceWith($('<a/>', {
        'data-toggle': 'modal',
    ...
    manageMode = vars('manage_mode'); // Initialize page's components (tooltips, datepickers etc).

    tippy('[data-tippy-content]');
    var weekdayId = App.Utils.Date.getWeekdayId(vars('first_weekday'));
    $selectDate.datepicker({
      ...
      onChangeMonthYear: function onChangeMonthYear(year, month) {
        ...
        App.Http.Booking.getUnavailabilityDates($selectProvider.val(), $selectService.val(), moment(currentDate).format('YYYY-MM-DD'));
      }
    });
    $selectTimezone.val(Intl.DateTimeFormat().resolvedOptions().timeZone); // Bind the event handlers (might not be necessary every time we use this class).

    addEventListeners();
    optimizeContactInfoDisplay(); // If the manage mode is true, the appointments data should be loaded by default.

    if (manageMode) {
      applyAppointmentData(vars('appointment_data'), vars('provider_data'), vars('customer_data'));
    } else {
      // Check if a specific service was selected (via URL parameter).
      var selectedServiceId = App.Utils.Url.queryParam('service');
      ...
      var selectedProviderId = App.Utils.Url.queryParam('provider');

      if (selectedProviderId && $selectProvider.find('option[value="' + selectedProviderId + '"]').length === 0) {
        // Select a service of this provider in order to make the provider available in the select box.
        for (var index in vars('available_providers')) {
          var provider = vars('available_providers')[index];
      ...
      if (selectedServiceId && selectedProviderId || vars('available_services').length === 1 && vars('available_providers').length === 1) {
        ...
        $(document).find('.book-step:not(:first)').each(function (index, bookStepEl) {
          return $(bookStepEl).find('strong').text(index + 1);
        });
      }
  ...
  function optimizeContactInfoDisplay() {
    var $fieldCols = $(document).find('#wizard-frame-3 .field-col');
    $fieldCols.each(function (index, fieldColEl) {
      var $fieldCol = $(fieldColEl);
  ...
  function addEventListeners() {
    ...
    $selectProvider.on('change', function (event) {
      var $target = $(event.target);
      App.Http.Booking.getUnavailabilityDates($target.val(), $selectService.val(), moment($selectDate.datepicker('getDate')).format('YYYY-MM-DD'));
      updateConfirmFrame();
    });
    /**
    ...
    $selectService.on('change', function (event) {
      ...
      App.Http.Booking.getUnavailabilityDates($selectProvider.val(), $target.val(), moment($selectDate.datepicker('getDate')).format('YYYY-MM-DD'));
      updateConfirmFrame();
    ...
    $('.button-next').on('click', function (event) {
      var $target = $(event.target); // If we are on the first step and there is no provider selected do not continue with the next step.
      ...
      var nextTabIndex = parseInt($target.attr('data-step_index')) + 1;
      $target.parents().eq(1).hide('fade', function () {
    ...
    $('.button-back').on('click', function (event) {
      var prevTabIndex = parseInt($(event.target).attr('data-step_index')) - 1;
      $(event.target).parents().eq(1).hide('fade', function () {
        $('.active-step').removeClass('active-step');
    ...
    $availableHours.on('click', '.available-hour', function (event) {
    ...
    if (manageMode) {
      ...
      $('#cancel-appointment').on('click', function () {
        var $cancelAppointmentForm = $('#cancel-appointment-form');
        var $cancelReason;
        var buttons = [{
          text: lang('cancel'),
        ...
        App.Utils.Message.show(lang('cancel_appointment_title'), lang('write_appointment_removal_reason'), buttons);
      ...
      $deletePersonalInformation.on('click', function () {
        var buttons = [{
          text: lang('cancel'),
          click: function click() {
            $('#message-box').dialog('close');
        ...
        App.Utils.Message.show(lang('delete_personal_information'), lang('delete_personal_information_prompt'), buttons);
      });
    }
    /**
    ...
    $captchaTitle.on('click', 'button', function () {
      $('.captcha-image').attr('src', App.Utils.Url.siteUrl('captcha?' + Date.now()));
    });
    $selectDate.on('mousedown', '.ui-datepicker-calendar td', function () {
      setTimeout(function () {
  ...
  function updateConfirmFrame() {
    ...
    var selectedDate = $selectDate.datepicker('getDate');
    ...
    var servicePrice = '';
    var serviceCurrency = '';
    vars('available_services').forEach(function (service) {
    ...
    $(document).find('.display-selected-service').text($selectService.find('option:selected').text()).removeClass('invisible');
    $(document).find('.display-selected-provider').text($selectProvider.find('option:selected').text()).removeClass('invisible');
    ...
    $('<div/>', {
      'html': [$('<h4/>', {
        ...
        'html': [$('<span/>', {
          ...
          'text': lang('start') + ': ' + selectedDate + ' ' + $availableHours.find('.selected-hour').text()
        }), $('<br/>'), $('<span/>', {
    ...
    }).appendTo('#appointment-details'); // Customer Details

    var firstName = App.Utils.String.escapeHtml($firstName.val());
    var lastName = App.Utils.String.escapeHtml($lastName.val());
    var phoneNumber = App.Utils.String.escapeHtml($phoneNumber.val());
    var email = App.Utils.String.escapeHtml($email.val());
    var address = App.Utils.String.escapeHtml($address.val());
    var city = App.Utils.String.escapeHtml($city.val());
    var zipCode = App.Utils.String.escapeHtml($zipCode.val());
    ...
    }).appendTo('#customer-details'); // Update appointment form data for submission to server when the user confirms the appointment.
    ...
    data.appointment = {
      start_datetime: moment($selectDate.datepicker('getDate')).format('YYYY-MM-DD') + ' ' + moment($('.selected-hour').data('value'), 'HH:mm').format('HH:mm') + ':00',
  ...
  function calculateEndDatetime() {
    ...
    var selectedDate = moment($selectDate.datepicker('getDate')).format('YYYY-MM-DD');
    ...
    if (service.duration && startMoment) {
      endMoment = startMoment.clone().add({
  ...
  function applyAppointmentData(appointment, provider, customer) {
    try {
      ...
      $selectProvider.val(appointment.id_users_provider); // Set Appointment Date

      var startMoment = moment(appointment.start_datetime);
      $selectDate.datepicker('setDate', startMoment.toDate());
      App.Http.Booking.getAvailableHours(startMoment.format('YYYY-MM-DD')); // Apply Customer's Data
