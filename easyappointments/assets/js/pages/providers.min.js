...
App.Pages.Providers = function () {
  ...
  var $filterProviders = $('#filter-providers');
  var filterResults = {};
  var filterLimit = 20;
  var workingPlanManager;
  /**
  ...
  function addEventListeners() {
    ...
    $providers.on('click', '.provider-row', function (event) {
      ...
      var provider = filterResults.find(function (filterResult) {
    ...
    $providers.on('click', '#add-provider', function () {
      ...
      $('#provider-services input:checkbox').prop('disabled', false); // Apply default working plan

      var companyWorkingPlan = JSON.parse(vars('company_working_plan'));
    ...
    $providers.on('click', '#save-provider', function () {
      ...
      provider.services = [];
      $('#provider-services input:checkbox').each(function (index, checkboxEl) {
  ...
  function save(provider) {
    App.Http.Providers.save(provider).then(function (response) {
  ...
  function remove(id) {
    App.Http.Providers.destroy(id).then(function () {
  ...
  function validate() {
    ...
    try {
      ...
      $providers.find('.required').each(function (index, requiredFieldEl) {
  ...
  function display(provider) {
    ...
    $notifications.prop('checked', Boolean(Number(provider.settings.notifications))); // Add dedicated provider link.

    var dedicatedUrl = App.Utils.Url.siteUrl('?provider=' + encodeURIComponent(provider.id));
    var $link = $('<a/>', {
      'href': dedicatedUrl,
      'target': '_blank',
      'html': [$('<i/>', {
    ...
    provider.services.forEach(function (providerServiceId) {
      ...
      $checkbox.prop('checked', true); // Add dedicated service-provider link.

      dedicatedUrl = App.Utils.Url.siteUrl('?provider=' + encodeURIComponent(provider.id) + '&service=' + encodeURIComponent(providerServiceId));
      $link = $('<a/>', {
        'href': dedicatedUrl,
        'target': '_blank',
        'html': [$('<i/>', {
  ...
  function filter(keyword) {
    ...
    App.Http.Providers.search(keyword, filterLimit).then(function (response) {
      ...
      if (!response.length) {
        $filterProviders.find('.results').append($('<em/>', {
  ...
  function select(id) {
    ...
    $filterProviders.find('.provider-row[data-id="' + id + '"]').addClass('selected'); // Display record in form (if display = true).
