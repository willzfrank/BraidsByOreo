...
App.Pages.Secretaries = function () {
  ...
  var $filterSecretaries = $('#filter-secretaries');
  var filterResults = {};
  var filterLimit = 20;
  /**
  ...
  function addEventListeners() {
     ...
     */
    $secretaries.on('blur', '#username', function (event) {
      ...
      var secretaryId = $input.parents().eq(2).find('.record-id').val();
      ...
      App.Http.Account.validateUsername(secretaryId, username).done(function (response) {
    ...
    $secretaries.on('click', '.secretary-row', function (event) {
      ...
      var secretary = filterResults.find(function (filterResult) {
  ...
  function save(secretary) {
    App.Http.Secretaries.save(secretary).done(function (response) {
  ...
  function remove(id) {
    App.Http.Secretaries.destroy(id).done(function () {
  ...
  function validate() {
    ...
    try {
      ...
      $secretaries.find('.required').each(function (index, requiredField) {
  ...
  function filter(keyword) {
    ...
    App.Http.Secretaries.search(keyword, filterLimit).done(function (response) {
      ...
      if (!response.length) {
        $('#filter-secretaries .results').append($('<em/>', {
  ...
  function initialize() {
    ...
    filter('');
    addEventListeners();
    vars('providers').forEach(function (provider) {
      $('<div/>', {
        'class': 'checkbox',
        'html': [$('<div/>', {
