...
App.Utils.WorkingPlan = function () {
   ...
   * @class WorkingPlan
   */
  var WorkingPlan = /*#__PURE__*/function () {
    function WorkingPlan() {
      _classCallCheck(this, WorkingPlan);

      _defineProperty(this, "enableCancel", false);

      _defineProperty(this, "enableSubmit", false);
    }

    _createClass(WorkingPlan, [{
      key: "setup",
      value:
      /**
       ...
       */
      function setup(workingPlan) {
        var weekDayId = App.Utils.Date.getWeekdayId(vars('first_weekday'));
        var workingPlanSorted = App.Utils.Date.sortWeekDictionary(workingPlan, weekDayId);
        $('.working-plan tbody').empty();
        $('.breaks tbody').empty(); // Build working plan day list starting with the first weekday as set in the General settings
        ...
        $.each(workingPlanSorted, function (index, workingDay) {
          ...
          var dayDisplayName = App.Utils.String.upperCaseFirstLetter(day);
          $('<tr/>', {
            'html': [$('<td/>', {
              'html': [$('<div/>', {
          ...
          if (workingDay) {
            ...
            $('#' + index + '-end').val(moment(workingDay.end, 'HH:mm').format(timeFormat).toLowerCase()); // Sort day's breaks according to the starting hour
        ...
        }.bind(this)); // Make break cells editable.
      ...
      key: "setupWorkingPlanExceptions",
      value: function setupWorkingPlanExceptions(workingPlanExceptions) {
      ...
      key: "editableDayCell",
      value: function editableDayCell($selector) {
        var weekDays = {};
        ...
        $selector.editable(function (value) {
          return value;
        }, {
          type: 'select',
          data: weekDays,
      ...
      key: "editableTimeCell",
      value: function editableTimeCell($selector) {
        $selector.editable(function (value) {
      ...
      key: "renderWorkingPlanExceptionRow",
      value: function renderWorkingPlanExceptionRow(date, workingPlanException) {
        var timeFormat = vars('time_format') === 'regular' ? 'h:mm a' : 'HH:mm';
        return $('<tr/>', {
          'data': {
            'date': date,
      ...
      value: function addEventListeners() {
        ...
        $('.add-break').on('click', function () {
          ...
          }).appendTo('.breaks tbody'); // Bind editable and event handlers.
        ...
        $(document).on('click', '.edit-break', function () {
          ...
          $(this).parent().parent().find('.break-day select').focus(); // Show save - cancel buttons.
        ...
        $(document).on('click', '.save-break', function (event) {
          ...
          if (startMoment.isAfter(endMoment)) {
            $modifiedRow.find('.break-end input').val(startMoment.add(1, 'hour').format(vars('time_format') === 'regular' ? 'h:mm a' : 'HH:mm').toLowerCase());
          }
      ...
      value: function get() {
        ...
        $('.working-plan input:checkbox').each(function (index, checkbox) {
          ...
          if ($(checkbox).prop('checked') === true) {
            ...
            }.bind(this)); // Sort breaks increasingly by hour within day
      ...
      key: "getWorkingPlanExceptions",
      value: function getWorkingPlanExceptions() {
      ...
      key: "timepickers",
      value: function timepickers(disabled) {
          ...
          $('.working-plan input:text').timepicker({
            ...
            onSelect: function onSelect() {
              ...
              if (startMoment > endMoment) {
                $(this).parent().parent().find('.work-end').val(startMoment.add(1, 'hour').format(vars('time_format') === 'regular' ? 'h:mm a' : 'HH:mm').toLowerCase());
              }
            }
          });
        } else {
      ...
      key: "convertValueToDay",
      value: function convertValueToDay(value) {
        switch (value) {
          case 'sunday':
            return lang('sunday');
      ...
      key: "convertDayToValue",
      value: function convertDayToValue(day) {
        switch (day) {
          case lang('sunday'):
            return 'sunday';
