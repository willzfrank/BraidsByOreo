...
App.Utils.CalendarDefaultView = function () {
  ...
  function addEventListeners() {
     ...
     */
    $reloadAppointments.on('click', function () {
      var calendarView = fullCalendar.view;
      ...
      refreshCalendarAppointments($calendar, $selectFilterItem.val(), $selectFilterItem.find('option:selected').attr('type'), calendarView.currentStart, calendarView.currentEnd);
    });
    /**
    ...
    $calendarPage.on('click', '.close-popover', function (event) {
    ...
    $calendarPage.on('click', '.edit-popover', function (event) {
    ...
    $calendarPage.on('click', '.delete-popover', function (event) {
      var $target = $(event.target);
  ...
  function getCalendarHeight() {
    var result = window.innerHeight - $footer.outerHeight() - $header.outerHeight() - $calendarToolbar.outerHeight() - 60; // 60 for fine tuning
  ...
  function onEventClick(info) {
    ...
    var $html;
    var displayEdit;
    var displayDelete; // Depending on where the user clicked the event (title or empty space) we
    ...
    if ($target.hasClass('fc-unavailability')) {
      ...
      displayDelete = $target.hasClass('fc-custom') && vars('privileges').appointments["delete"] === true ? 'me-2' : 'd-none'; // Same value at the time.
      ...
      displayDelete = $target.hasClass('fc-custom') && vars('privileges').appointments["delete"] === true ? 'me-2' : 'd-none';
      $html = $('<div/>', {
        'html': [$('<strong/>', {
          'class': 'd-inline-block me-2',
          'text': lang('provider')
    ...
    var $popover = $calendarPage.find('.popover');
  ...
  function onEventResize(info) {
    ...
    if (!info.event.extendedProps.data.is_unavailability) {
      ...
      var appointment = _objectSpread({}, info.event.extendedProps.data);

      appointment.is_unavailability = Number(appointment.is_unavailability); // Must delete the following because only appointment data should be provided to the AJAX call.
      ...
      successCallback = function successCallback() {
        ...
        var undoFunction = function undoFunction() {
          ...
          App.Http.Calendar.saveAppointment(unavailability).done(function () {
            $notification.hide('blind');
          });
          info.revert();
        };
  ...
  function onEventDrop(info) {
    ...
    if (!info.event.extendedProps.data.is_unavailability) {
      ...
      appointment.start_datetime = moment(appointment.start_datetime).add({
        days: info.delta.days,
        millisecond: info.delta.milliseconds
      }).format('YYYY-MM-DD HH:mm:ss');
  ...
  function onDatesSet() {
    ...
    refreshCalendarAppointments($calendar, $selectFilterItem.val(), $('#select-filter-item option:selected').attr('type'), fullCalendar.view.currentStart, fullCalendar.view.currentEnd);
    ...
    $('.fv-events').each(function (index, eventEl) {
      $(eventEl).popover();
    });
  } // /**
  ...
  function refreshCalendarAppointments($calendar, recordId, filterType, startDate, endDate) {
    ...
    App.Http.Calendar.getCalendarAppointments(recordId, startDate, endDate, filterType).done(function (response) {
      var calendarEventSources = fullCalendar.getEventSources();
      calendarEventSources.forEach(function (calendarEventSource) {
        return calendarEventSource.remove();
      ...
      response.unavailabilities.forEach(function (unavailability) {
        var notes = unavailability.notes ? ' - ' + unavailability.notes : '';
      ...
      if (filterType === FILTER_TYPE_PROVIDER && calendarView.type !== 'dayGridMonth') {
        var _ret = function () {
          ...
          var endHour;
          var workDateStart;
          var workDateEnd; // Sort the working plan starting with the first day as set in General settings to correctly align
          // breaks in the calendar display.

          var firstWeekdayNumber = App.Utils.Date.getWeekdayId(vars('first_weekday'));
          var sortedWorkingPlan = App.Utils.Date.sortWeekDictionary(workingPlan, firstWeekdayNumber);
          ...
          var _ret2 = function () {
            switch (calendarView.type) {
              case 'timeGridDay':
                ...
                weekdayDate = moment(calendarView.currentStart).clone().format('YYYY-MM-DD'); // Add working plan exception.
                ...
                if (viewStart < workDateStart) {
                  var unavailabilityPeriodBeforeWorkStarts = {
                    title: lang('not_working'),
                    start: viewStart,
                    end: workDateStart,
                    allDay: false,
                ...
                if (viewEnd > workDateEnd) {
                  var unavailabilityPeriodAfterWorkEnds = {
                    title: lang('not_working'),
                    start: workDateEnd,
                    end: viewEnd,
                    allDay: false,
              ...
              case 'timeGridWeek':
                var calendarDate = moment(calendarView.currentStart).clone();

                while (calendarDate.toDate() < calendarView.currentEnd) {
                  weekdayNumber = parseInt(calendarDate.format('d'));
                  weekdayName = App.Utils.Date.getWeekdayName(weekdayNumber);
                  weekdayDate = calendarDate.format('YYYY-MM-DD'); // Add working plan exception event.
                  ...
                  if (calendarDate.toDate() < workDateStart.toDate()) {
                    unavailabilityEvent = {
                      title: lang('not_working'),
                      start: calendarDate.clone().toDate(),
                      end: moment(calendarDate.format('YYYY-MM-DD') + ' ' + sortedWorkingPlan[weekdayName].start + ':00').toDate(),
                      allDay: false,
                      color: '#BEBEBE',
                      editable: false,
                      className: 'fc-unavailability'
                  ...
                  if (calendarView.currentEnd > workDateEnd.toDate()) {
                    unavailabilityEvent = {
                      title: lang('not_working'),
                      start: moment(calendarDate.format('YYYY-MM-DD') + ' ' + sortedWorkingPlan[weekdayName].end + ':00').toDate(),
                      end: calendarDate.clone().add(1, 'day').toDate(),
                      allDay: false,
                      color: '#BEBEBE',
                      editable: false,
  ...
  function initialize() {
    ...
    var initialView = window.innerWidth < 468 ? 'timeGridDay' : 'timeGridWeek';
    var firstWeekday = vars('first_weekday');
    var firstWeekdayNumber = App.Utils.Date.getWeekdayId(firstWeekday); // Initialize page calendar
    ...
    fullCalendar = new FullCalendar.Calendar($calendar[0], {
      ...
      select: function select(info) {
        ...
        var isProviderDisplayed = $selectFilterItem.find('option:selected').attr('type') === FILTER_TYPE_PROVIDER;
        var buttons = [{
          text: lang('close'),
          click: function click() {
            $('#message-box').dialog('close');
          ...
          click: function click() {
            ...
            if (isProviderDisplayed) {
              var provider = vars('available_providers').find(function (availableProvider) {
                return Number(availableProvider.id) === Number($selectFilterItem.val());
              });

              if (provider) {
                service = vars('available_services').find(function (availableService) {
                  return provider.services.indexOf(availableService.id) !== -1;
                });
              ...
              service = vars('available_services').find(function (availableService) {
                return Number(availableService.id) === Number($selectFilterItem.val());
              });
    ...
    fullCalendar.render(); // Trigger once to set the proper footer position after calendar initialization.

    onWindowResize();
    $selectFilterItem.append(new Option(lang('all'), 'all', true, true)); // Fill the select list boxes of the page.

    if (vars('available_providers').length > 0) {
      $('<optgroup/>', {
        'label': lang('providers'),
        'type': 'providers-group',
        'html': vars('available_providers').map(function (availableProvider) {
    ...
    if (vars('available_services').length > 0) {
      $('<optgroup/>', {
        'label': lang('services'),
        'type': 'services-group',
        'html': vars('available_services').map(function (availableService) {
          return $('<option/>', {
    ...
    if (vars('role_slug') === App.Layouts.Backend.DB_SLUG_PROVIDER) {
      $selectFilterItem.find('optgroup:eq(0)').find('option[value="' + vars('user_id') + '"]').prop('selected', true);
    } // Add the page event listeners.


    addEventListeners();
    $reloadAppointments.trigger('click'); // Display the edit dialog if an appointment hash is provided.
    ...
    if (vars('edit_appointment')) {
      ...
      var startDatetimeMoment = moment(appointment.start_datetime);
      $appointmentsModal.find('#start-datetime').datetimepicker('setDate', startDatetimeMoment.toDate());
      var endDatetimeMoment = moment(appointment.end_datetime);
    ...
    setInterval(function () {
      ...
      refreshCalendarAppointments($calendar, $selectFilterItem.val(), $selectFilterItem.find('option:selected').attr('type'), fullCalendar.view.currentStart, fullCalendar.view.currentEnd);
    }, 60000);
    $reloadAppointments.trigger('click');
  }
